...
(function (root) {
  ...
  async function callGemini({ text, image, refDateISO, apiKey, listNames, mode, userEmail, signal }) {
    ...
    // The pinned model answers in seconds with thinking off. The -latest alias
    // is only a survival fallback: it now points at Gemini 3+ Flash, which
    // thinks by default and blows straight through the 25s timeout (level
    // 'low' is the closest it has to off, and it rejects thinkingBudget).
    let res = await call('gemini-2.5-flash', { thinkingBudget: 0 });
