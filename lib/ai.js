...
//   - 'deepseek': DeepSeek (api.deepseek.com; text only, its chat API takes no images)
...
(function (root) {
  ...
  async function loadKeys() {
    const local = await chrome.storage.local.get({
      geminiApiKey: '', anthropicApiKey: '', openaiApiKey: '', kimiApiKey: '', deepseekApiKey: '',
    });
  ...
  const COMPAT_PROVIDERS = {
    ...
    kimi: {
      ...
      model: 'kimi-k3', // kimi-latest is gone from the current model list (July 2026); k3 is the vision-capable flagship
    },
    deepseek: {
      label: 'DeepSeek',
      urls: ['https://api.deepseek.com/chat/completions'],
      model: 'deepseek-v4-flash', // deepseek-chat was retired July 2026
      textOnly: true, // its chat API rejects image content
    },
  };
  ...
  async function callCompat(provider, { text, image, refDateISO, apiKey, listNames, signal }) {
    ...
    if (image && p.textOnly) {
      throw new Error(p.label + ' cannot read screenshots. Use another provider for images.');
    }
    const content = [];
    if (image) {
      content.push({
  ...
  async function extract({ text, image, refDateISO, apiKey, provider, listNames }) {
    ...
    try {
      ...
      raw =
        ...
        : provider === 'openai' || provider === 'kimi' || provider === 'deepseek' ? await callCompat(provider, args)
  ...
  // Screenshot flows must not be routed to a provider whose API takes no images
  const supportsImages = (p) => !(COMPAT_PROVIDERS[p] && COMPAT_PROVIDERS[p].textOnly);
  root.AiExtract = { extract, builtinAvailability, builtinImageAvailability, ensureBuiltinReady, loadKeys, supportsImages };
