...
(function (root) {
  ...
  // —— Anthropic Claude ——
  async function callClaude({ text, image, refDateISO, apiKey, listNames, mode, signal }) {
    const content = [];
    if (image) {
      content.push({
        type: 'image',
    ...
    const res = await doFetch('https://api.anthropic.com/v1/messages', {
      ...
      body: JSON.stringify({
        ...
        max_tokens: 1600,
        system: systemPrompt(refDateISO, listNames, mode),
  ...
  async function callGemini({ text, image, refDateISO, apiKey, listNames, mode, signal }) {
    const parts = [];
    ...
    const body = JSON.stringify({
      systemInstruction: { parts: [{ text: systemPrompt(refDateISO, listNames, mode) }] },
  ...
  async function callCompat(provider, { text, image, refDateISO, apiKey, listNames, mode, signal }) {
  ...
  async function callHosted({ text, image, refDateISO, apiKey, listNames, mode, signal }) {
    ...
    if (!res.ok) {
      console.warn('hosted extract failed with status', res.status);
  ...
  async function extract({ text, image, refDateISO, apiKey, provider, listNames, mode, signal }) {
    ...
    let raw;
    try {
      const args = { text, image, refDateISO, apiKey, listNames, mode, signal: controller.signal };
      raw =
