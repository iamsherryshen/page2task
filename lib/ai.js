...
// Returns { items: [{ title, dueDate, dueTime }] }, most important first.
(function (root) {
  // Most input is already short: page text and email bodies are cut to 6000
  // before they get here. The large cap is for a whole PDF syllabus, which runs
  // to tens of thousands of characters and keeps its deadlines deep inside.
  // The hosted relay applies the same cap on its side (docs/api/extract.js).
  // Chrome's on-device model has a small context window and keeps the old cap.
  const CLOUD_TEXT_CAP = 24000;
  const LOCAL_TEXT_CAP = 6000;
  // Far above any single syllabus (the largest of 19 real ones had 30 deliverables):
  // a guard against a runaway response, not a limit anyone should meet.
  const MAX_ITEMS = 60;
  ...
  async function callClaude({ text, image, refDateISO, apiKey, listNames, mode, userEmail, signal }) {
    ...
    content.push({ type: 'text', text: text ? String(text).slice(0, CLOUD_TEXT_CAP) : 'Extract from this screenshot.' });
  ...
  async function callGemini({ text, image, refDateISO, apiKey, listNames, mode, userEmail, signal }) {
    ...
    parts.push({ text: text ? String(text).slice(0, CLOUD_TEXT_CAP) : 'Extract from this screenshot.' });
  ...
  async function callHosted({ text, image, refDateISO, apiKey, listNames, mode, userEmail, signal }) {
    const res = await doFetch(HOSTED_URL, {
      ...
      headers: { authorization: 'Bearer ' + apiKey, 'content-type': 'application/json' },
      body: JSON.stringify({
        text: text ? String(text).slice(0, CLOUD_TEXT_CAP) : null,
        image: image || null,
        refDateISO,
        listNames,
        mode: mode || null,
  ...
  async function callBuiltin({ text, image, refDateISO, signal }) {
    ...
    try {
      ...
      if (!image) return await session.prompt(String(text || '').slice(0, LOCAL_TEXT_CAP), promptOpts);
      ...
      return await session.prompt(
        String(transcript || '').slice(0, LOCAL_TEXT_CAP) +
  ...
  async function extract({ text, image, refDateISO, apiKey, provider, listNames, mode, userEmail, signal }) {
    ...
    // A whole syllabus can hold dozens of deadlines, and writing them all out takes
    // far longer than the handful a web page has.
    const longText = !!text && String(text).length > 6000;
    const timer = setTimeout(
      () => controller.abort(),
      provider === 'builtin' ? (image ? 150000 : 90000) : image ? 45000 : longText ? 60000 : 25000
    );
    let raw;
    try {
    ...
    const items = parsed.items
      ...
      .slice(0, MAX_ITEMS); // safety valve, not a target: the prompt asks for every item
    return { items };
  }
